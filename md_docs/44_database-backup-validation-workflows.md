# How To: Database Backup Validation Workflows
...
## Overview
This document outlines procedure steps for **Database Backup Validation Workflows** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Disaster Recovery, Active Network Access.
...
atlas backups snapshots list --sourceID=SRC-99402 --env=prod
```
...
```bash
atlas backups restore start --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Database Backup Validation Workflows...' && atlas backups restore start
```
...
### Common Error
**Error Message**: `Error: DisasterRecoveryExecutionFailed`
