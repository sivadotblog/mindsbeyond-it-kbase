# How To: Service Account Key Auto-Rotation
...
## Overview
This document outlines procedure steps for **Service Account Key Auto-Rotation** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Security Automation, Active Network Access.
...
rotate_keys.py --sa=sa-kbase --sourceID=SRC-99402 --env=prod
```
...
```bash
az keyvault secret show --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Service Account Key Auto-Rotation...' && az keyvault secret show
```
...
### Common Error
**Error Message**: `Error: SecurityAutomationExecutionFailed`
