# How To: API Gateway Rate Limit Enforcement
...
## Overview
This document outlines procedure steps for **API Gateway Rate Limit Enforcement** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for API Security, Active Network Access.
...
apim policy rate-limit set --sourceID=SRC-99402 --env=prod
```
...
```bash
curl -i https://api.mindsbeyond.com --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying API Gateway Rate Limit Enforcement...' && curl -i https://api.mindsbeyond.com
```
...
### Common Error
**Error Message**: `Error: APISecurityExecutionFailed`
