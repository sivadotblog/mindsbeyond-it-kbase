# How To: Atlas Storage Auto-Expansion Tuning
...
## Overview
This document outlines procedure steps for **Atlas Storage Auto-Expansion Tuning** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Storage Ops, Active Network Access.
...
atlas cluster storage enable-auto --sourceID=SRC-99402 --env=prod
```
...
```bash
atlas cluster show --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Atlas Storage Auto-Expansion Tuning...' && atlas cluster show
```
...
### Common Error
**Error Message**: `Error: StorageOpsExecutionFailed`
