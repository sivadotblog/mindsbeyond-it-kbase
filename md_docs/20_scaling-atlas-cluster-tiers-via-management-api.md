# How To: Scaling Atlas Cluster Tiers via Management API
...
## Overview
This document outlines procedure steps for **Scaling Atlas Cluster Tiers via Management API** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Atlas Project Owner API Keys.
...
curl -X PATCH https://cloud.mongodb.com/api/atlas/v2/groups/$ATLAS_PROJECT_ID/clusters/$CLUSTER -H 'Content-Type: application/json' -d '{"providerSettings":{"instanceSizeName":"M30"}}'
```
...
```bash
curl https://cloud.mongodb.com/api/atlas/v2/groups/$ATLAS_PROJECT_ID/clusters/$CLUSTER
```
...
```bash
mongosh $MONGODB_URI --eval 'db.hostInfo()'
```
...
### Common Error
**Error Message**: `CANNOT_DOWNGRADE_BELOW_STORAGE`

**Resolution**: Ensure requested cluster tier supports the currently provisioned storage size.
