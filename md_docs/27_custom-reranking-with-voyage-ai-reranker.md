# How To: Custom Reranking with Voyage AI Reranker
...
## Overview
This document outlines procedure steps for **Custom Reranking with Voyage AI Reranker** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for AI Search, Active Network Access.
...
rerank_pipeline.py --sourceID=SRC-99402 --env=prod
```
...
```bash
vo.rerank(query, docs) --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Custom Reranking with Voyage AI Reranker...' && vo.rerank(query, docs)
```
...
### Common Error
**Error Message**: `Error: AISearchExecutionFailed`
