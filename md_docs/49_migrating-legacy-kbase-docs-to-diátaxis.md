# How To: Migrating Legacy KBase Docs to Diátaxis
...
## Overview
This document outlines procedure steps for **Migrating Legacy KBase Docs to Diátaxis** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Knowledge Mgmt, Active Network Access.
...
python convert_to_diataxis.py --sourceID=SRC-99402 --env=prod
```
...
```bash
pytest test_structure.py --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Migrating Legacy KBase Docs to Diátaxis...' && pytest test_structure.py
```
...
### Common Error
**Error Message**: `Error: KnowledgeMgmtExecutionFailed`
