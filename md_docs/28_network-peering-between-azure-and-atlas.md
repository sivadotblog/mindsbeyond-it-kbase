# How To: Network Peering Between Azure and Atlas
...
## Overview
This document outlines procedure steps for **Network Peering Between Azure and Atlas** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Networking, Active Network Access.
...
az network vnet peering create --sourceID=SRC-99402 --env=prod
```
...
```bash
az network vnet peering show --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Network Peering Between Azure and Atlas...' && az network vnet peering show
```
...
### Common Error
**Error Message**: `Error: NetworkingExecutionFailed`
