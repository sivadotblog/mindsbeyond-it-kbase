# How To: Zero-Downtime Migration Procedures
...
## Overview
This document outlines procedure steps for **Zero-Downtime Migration Procedures** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Database Admin, Active Network Access.
...
mongodump --oplog --sourceID=SRC-99402 --env=prod
```
...
```bash
mongorestore --oplogReplay --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Zero-Downtime Migration Procedures...' && mongorestore --oplogReplay
```
...
### Common Error
**Error Message**: `Error: DatabaseAdminExecutionFailed`
