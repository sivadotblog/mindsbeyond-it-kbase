# How To: Kafka Schema Registry Enforcement
...
## Overview
This document outlines procedure steps for **Kafka Schema Registry Enforcement** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Event Streaming, Active Network Access.
...
schema-registry-cli register --sourceID=SRC-99402 --env=prod
```
...
```bash
schema-registry-cli check --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Kafka Schema Registry Enforcement...' && schema-registry-cli check
```
...
### Common Error
**Error Message**: `Error: EventStreamingExecutionFailed`
