# How To: Snowflake External Tables Setup
...
## Overview
This document outlines procedure steps for **Snowflake External Tables Setup** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Data Warehouse, Active Network Access.
...
snowflake-cli --sourceID=SRC-99402 --env=prod
```
...
```bash
SELECT * FROM mongo_ext_table --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Snowflake External Tables Setup...' && SELECT * FROM mongo_ext_table
```
...
### Common Error
**Error Message**: `Error: DataWarehouseExecutionFailed`
