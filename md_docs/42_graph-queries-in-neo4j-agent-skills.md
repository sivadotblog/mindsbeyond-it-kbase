# How To: Graph Queries in Neo4j Agent Skills
...
## Overview
This document outlines procedure steps for **Graph Queries in Neo4j Agent Skills** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Graph Analytics, Active Network Access.
...
cypher-shell MATCH (n) RETURN n --sourceID=SRC-99402 --env=prod
```
...
```bash
python test_graph_agent.py --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Graph Queries in Neo4j Agent Skills...' && python test_graph_agent.py
```
...
### Common Error
**Error Message**: `Error: GraphAnalyticsExecutionFailed`
