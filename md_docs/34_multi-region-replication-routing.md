# How To: Multi-Region Replication Routing
...
## Overview
This document outlines procedure steps for **Multi-Region Replication Routing** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for High Availability, Active Network Access.
...
atlas cluster update --regions --sourceID=SRC-99402 --env=prod
```
...
```bash
mongosh --host replSet/node1,node2 --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Multi-Region Replication Routing...' && mongosh --host replSet/node1,node2
```
...
### Common Error
**Error Message**: `Error: HighAvailabilityExecutionFailed`
