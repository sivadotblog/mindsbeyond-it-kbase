# How To: MongoDB Shell Scripting for Batch Updates
...
## Overview
This document outlines procedure steps for **MongoDB Shell Scripting for Batch Updates** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for DB Ops, Active Network Access.
...
mongosh batch_update.js --sourceID=SRC-99402 --env=prod
```
...
```bash
mongosh verify_update.js --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying MongoDB Shell Scripting for Batch Updates...' && mongosh verify_update.js
```
...
### Common Error
**Error Message**: `Error: DBOpsExecutionFailed`
