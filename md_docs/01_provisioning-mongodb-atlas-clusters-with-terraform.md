# How To: Provisioning MongoDB Atlas Clusters with Terraform
...
## Overview
This document outlines procedure steps for **Provisioning MongoDB Atlas Clusters with Terraform** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Terraform CLI 1.5+, Atlas Programmatic API Keys.
...
terraform init && terraform plan -var-file=mindsbeyond-prod.tfvars
```
...
```bash
terraform apply -auto-approve -var-file=mindsbeyond-prod.tfvars
```
...
```bash
az network private-endpoint show -g rg-mindsbeyond-data -n pe-atlas-eastus
```
...
**Resolution**: Verify ATLAS_PUBLIC_KEY and ATLAS_PRIVATE_KEY in environment variables.
