# How To: Configuring TTL Indexes for Expiring Enterprise Logs
...
## Overview
This document outlines procedure steps for **Configuring TTL Indexes for Expiring Enterprise Logs** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: MongoDB Admin Access on kbase database.
...
mongosh $MONGODB_URI --eval 'db.system_logs.createIndex({ "createdAt": 1 }, { expireAfterSeconds: 2592000 })'
```
...
```bash
mongosh $MONGODB_URI --eval 'db.system_logs.getIndexes()'
```
...
```bash
mongosh $MONGODB_URI --eval 'db.system_logs.stats().indexSizes'
```
...
### Common Error
**Error Message**: `IndexOptionsConflict`

**Resolution**: Drop existing index on createdAt before creating TTL index with expireAfterSeconds.
