# How To: End-to-End RAG Testing for KBase Agents
...
## Overview
This document outlines procedure steps for **End-to-End RAG Testing for KBase Agents** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for AI Validation, Active Network Access.
...
pytest test_rag_accuracy.py --sourceID=SRC-99402 --env=prod
```
...
```bash
python eval_metrics.py --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying End-to-End RAG Testing for KBase Agents...' && python eval_metrics.py
```
...
### Common Error
**Error Message**: `Error: AIValidationExecutionFailed`
