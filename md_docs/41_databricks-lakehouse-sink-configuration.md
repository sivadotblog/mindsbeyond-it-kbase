# How To: Databricks Lakehouse Sink Configuration

**Document ID**: KB-0041  
**Category**: Tutorial  
**Domain**: Lakehouse Ops  
**Framework**: Diátaxis Framework  

---

## Overview
This document outlines procedure steps for **Databricks Lakehouse Sink Configuration** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Lakehouse Ops, Active Network Access.
...
spark-submit sink_job.py --sourceID=SRC-99402 --env=prod
```
...
```bash
databricks runs get --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Databricks Lakehouse Sink Configuration...' && databricks runs get
```
...
### Common Error
**Error Message**: `Error: LakehouseOpsExecutionFailed`
