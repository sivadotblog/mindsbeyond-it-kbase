# How To: Private DNS Zone Linking for Atlas
...
## Overview
This document outlines procedure steps for **Private DNS Zone Linking for Atlas** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for DNS Operations, Active Network Access.
...
az network private-dns link create --sourceID=SRC-99402 --env=prod
```
...
```bash
dig SRV mongo.mindsbeyond.com --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Private DNS Zone Linking for Atlas...' && dig SRV mongo.mindsbeyond.com
```
...
### Common Error
**Error Message**: `Error: DNSOperationsExecutionFailed`
