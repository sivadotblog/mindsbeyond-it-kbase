# How To: Data Platform Alert Webhooks Integration
...
## Overview
This document outlines procedure steps for **Data Platform Alert Webhooks Integration** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Alerting, Active Network Access.
...
curl -X POST webhook.mindsbeyond.com --sourceID=SRC-99402 --env=prod
```
...
```bash
curl -X GET webhook.mindsbeyond.com/status --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Data Platform Alert Webhooks Integration...' && curl -X GET webhook.mindsbeyond.com/status
```
...
### Common Error
**Error Message**: `Error: AlertingExecutionFailed`
