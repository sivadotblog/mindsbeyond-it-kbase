# How To: Azure AD Single Sign-On Configuration
...
## Overview
This document outlines procedure steps for **Azure AD Single Sign-On Configuration** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Identity, Active Network Access.
...
az ad app create --sourceID=SRC-99402 --env=prod
```
...
```bash
az ad sso show --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Azure AD Single Sign-On Configuration...' && az ad sso show
```
...
### Common Error
**Error Message**: `Error: IdentityExecutionFailed`
