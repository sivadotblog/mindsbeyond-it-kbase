# How To: Atlas Memory Overhead Monitoring
...
## Overview
This document outlines procedure steps for **Atlas Memory Overhead Monitoring** in the internal Mindsbeyond Data Platform environment.
...
1. **Tooling & Access**: Enterprise Credentials for Performance, Active Network Access.
...
mongosh --eval 'db.serverStatus().mem' --sourceID=SRC-99402 --env=prod
```
...
```bash
mongosh --eval 'db.stats()' --config=/etc/mindsbeyond/config.json
```
...
```bash
echo 'Verifying Atlas Memory Overhead Monitoring...' && mongosh --eval 'db.stats()'
```
...
### Common Error
**Error Message**: `Error: PerformanceExecutionFailed`
